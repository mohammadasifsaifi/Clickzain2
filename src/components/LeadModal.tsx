...
import { X, User, Mail, Building2, Wallet, Send, CheckCircle2, Loader2, Phone, MessageSquare, Zap } from 'lucide-react';
...
const LeadModal = ({ isOpen, onClose, onSuccess }: LeadModalProps) => {
  ...
  const validate = () => {
    ...
    if (!formData.monthlyBudget) newErrors.monthlyBudget = "Please select a budget";
    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            setStatus('idle');
            setFormData({ fullName: '', phone: '', email: '', businessName: '', monthlyBudget: '', message: '' });
          }, 500);
        }, 1500);
      } else {
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="fixed inset-0 pointer-events-none overflow-hidden">
            ...
            <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-brand-secondary/10 blur-[120px] rounded-full animate-pulse" style={{ animationDelay: '2s' }} />
          </div>
          ...
          <div className="relative z-10 flex justify-between items-center px-6 py-8 md:px-12">
            ...
            <div className="hidden md:block">
              <div className="text-2xl font-display font-black tracking-tighter text-white">CLICKZAIN</div>
            </div>
          </div>
            ...
            <div className="w-full max-w-7xl grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
              ...
              >
                ...
                <h2 className="text-6xl xl:text-7xl font-display font-black text-white mb-8 leading-[1.1] tracking-tighter">
                  Let’s Scale Your Business with <span className="gradient-text">Smart Ads 🚀</span>
                </h2>
                <p className="text-white/60 text-xl mb-12 max-w-lg leading-relaxed">
                  Stop guessing and start growing. Our AI-driven systems find your ideal customers and convert them into high-value leads automatically.
                </p>
                
                <div className="space-y-6">
                  {[
                    { icon: CheckCircle2, text: "Data-Driven Audience Targeting" },
                    { icon: CheckCircle2, text: "AI-Optimized Ad Creatives" },
                    { icon: CheckCircle2, text: "High-Conversion Landing Pages" },
                    { icon: CheckCircle2, text: "End-to-End Campaign Management" }
                  ].map((item, idx) => (
        ...
        <h3 className="text-4xl font-display font-bold text-white mb-6">✅ Thank you!</h3>
        <p className="text-white/50 text-lg mb-12">
          {status === 'success' && errorMessage ? errorMessage : "Our team will contact you shortly to schedule your free strategy call."}
        </p>
                        <button 
                          onClick={handleReset}
                      ...
                      <motion.div key="form">
                        <div className="lg:hidden mb-10">
                          <h2 className="text-4xl font-display font-black text-white mb-4 tracking-tighter">LET'S SCALE 🚀</h2>
                          <p className="text-white/40 text-sm uppercase tracking-widest font-bold">Get your free AI marketing audit</p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                          <div className="grid md:grid-cols-2 gap-5">
                            <div className="relative group">
                              ...
                              />
                              {errors.fullName && <p className="text-[10px] text-red-500 font-bold uppercase mt-1 ml-2">{errors.fullName}</p>}
                            </div>
                            ...
                            <div className="relative group">
                              ...
                              <input
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-14 pr-6 text-white outline-none focus:border-brand-primary/50 focus:bg-white/[0.08] transition-all placeholder:text-white/20"
                              />
                              {errors.phone && <p className="text-[10px] text-red-500 font-bold uppercase mt-1 ml-2">{errors.phone}</p>}
                            </div>
                          </div>
                          ...
                          <div className="relative group">
                            ...
                            <input
                              ...
                              className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-14 pr-6 text-white outline-none focus:border-brand-primary/50 focus:bg-white/[0.08] transition-all placeholder:text-white/20"
                            />
                            {errors.email && <p className="text-[10px] text-red-500 font-bold uppercase mt-1 ml-2">{errors.email}</p>}
                          </div>
                          ...
                          <div className="relative group">
                            ...
                            <input
                              ...
                              className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-14 pr-6 text-white outline-none focus:border-brand-primary/50 focus:bg-white/[0.08] transition-all placeholder:text-white/20"
                            />
                            {errors.businessName && <p className="text-[10px] text-red-500 font-bold uppercase mt-1 ml-2">{errors.businessName}</p>}
                          </div>
                          ...
                          <div className="relative group">
                            ...
                            >
                              <option value="" disabled className="bg-[#0a0a0a]">Monthly Ads Budget</option>
                              <option value="₹10K – ₹30K" className="bg-[#0a0a0a]">₹10K – ₹30K</option>
                              <option value="₹30K – ₹70K" className="bg-[#0a0a0a]">₹30K – ₹70K</option>
                              <option value="₹70K – ₹1 Lakh" className="bg-[#0a0a0a]">₹70K – ₹1 Lakh</option>
                              <option value="₹1 Lakh+" className="bg-[#0a0a0a]">₹1 Lakh+</option>
                            </select>
                            {errors.monthlyBudget && <p className="text-[10px] text-red-500 font-bold uppercase mt-1 ml-2">{errors.monthlyBudget}</p>}
                          </div>
